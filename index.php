<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>NHL WEBSHOP</title>
    <link href="font/fontawesome-free-6.0.0-web/css/svg-with-js.css">
    <link rel="stylesheet" href="css/jquery.bxslider.min.css">
    <link rel="stylesheet" href="css/owl.carousel.min.css">
    <link rel="stylesheet" href="css/owl.theme.default.min.css">
    <link rel="stylesheet" href="css/bootstrap.min.css">
    <link rel="stylesheet" href="css/style.css">
</head>
<body>
    <div class="wrapper">
        <div class="navigation-bar">
            <div class="container">
                 <!-- insert any logo -->
                <div class="logo">
                   <img src="images/NHL webshop 1.PNG" alt="" width="87px">
                </div>
                <!-- This is a search box -->
                <div class="search-area">
                    <form action="" method="post">
                        <input type="text" name="search_box" class="search_box" placeholder="Search all the items..">
                        <button class="search_button"><i class="fa-solid fa-magnifying-glass"></i></button>
                    </form>
                </div>
                <!-- User Menu -->
                <div class="user-menu">
                    <ul>
                        <li class="active">
                            <a href="#"><i class="fas fa-user"></i>My account</a>
                            <!-- dropdown box -->
                            <div class="dropdown-menu"> <!-- dropdown-menu-right bg-dark" -->
                                <ul>
                                    <li><a href="#">Sign in</a></li>
                                    <li><a href="#">Order</a></li>
                                    <li><a href="#">Cart</a></li>
                                </ul>
                            </div>
                        </li>
                        <li><a href="#"><i class="fa-solid fa-cart-shopping"></i>Cart</a></li>
                    </ul>
                </div>
            </div>
        </div>
    </div>
    

<i class="fas font"></i>
<script src="js/jquery.js"></script>
<script src="js/jquery.bxslider.min.js"></script>
<script src="js/owl.carousel.min.js"></script>
<script src="js/bootstrap.bundle.min.js"></script>
<script src="js/bootstrap.min.js"></script>
<script src="font/fontawesome-free-6.0.0-web/js/all.js"></script>
<script src="js/main.js"></script>
</body>
</html>